...
**Replay** (`GNNBuffer` in `sac/common/gnn_buffer.py`) is a set of task-balanced
circular buffers, one per topology.

- Each entry stores PyG observation and next-observation graphs, node-aligned
  actions, the reward, and the termination flag, all on the CPU.
- Every batch draws `batch_size / num_topologies` samples from each topology and
  collates them into a PyG `Batch`.
