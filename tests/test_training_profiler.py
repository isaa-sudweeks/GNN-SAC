...
class TrainingProfilerTest(unittest.TestCase):
    ...
    def test_from_config_resolves_output_under_work_dir(self):
        with TemporaryDirectory() as tmp:
            ...
            self.assertFalse(profiler.optimization_subphases_enabled)
    ...
    def test_disabled_optimizer_subphases_do_not_synchronize_or_record(self):
        with TemporaryDirectory() as tmp:
            profiler = TrainingProfiler(
                enabled=True,
                warmup_vector_steps=0,
            ...
            profiler.begin_vector_step(global_step=0)
            with profiler.optimization_subphase("pcgrad_actor_projection"):
                pass
            profiler.end_vector_step(
                transitions=1,
                optimizer_updates=1,
            ...
            self.assertEqual(
                profiler.summary(global_step=1)["optimization_subphases"],
                {},
            )
